...
export default function BagsLookbook() {
  ...
  const nextBag = () => {
    handleIndexChange((activeIndex + 1) % bagsData.length);
  };

  const prevBag = () => {
    handleIndexChange((activeIndex - 1 + bagsData.length) % bagsData.length);
  };

  // Keyboard navigation
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      if (e.key === "ArrowRight") nextBag();
      if (e.key === "ArrowLeft") prevBag();
    };
      ...
      <div className="max-w-[1700px] mx-auto w-full px-6 md:px-12 flex flex-col flex-1 relative">
        ...
        <div className="flex items-center justify-end mb-8 z-20 relative w-full">
          <div className="hidden lg:flex items-center gap-4 text-[#111111] font-bold text-sm tracking-widest">
            <button aria-label="Previous bag" onClick={prevBag} className="hover:text-[#FF2400] transition-colors w-10 h-10 flex items-center justify-center border border-[#111111]/10 rounded-full hover:border-[#FF2400]"><ArrowLeft className="w-4 h-4" /></button>
            <span className="min-w-[60px] text-center">{activeBag.id} <span className="text-[#111111]/30">/ 05</span></span>
            <button aria-label="Next bag" onClick={nextBag} className="hover:text-[#FF2400] transition-colors w-10 h-10 flex items-center justify-center border border-[#111111]/10 rounded-full hover:border-[#FF2400]"><ArrowRight className="w-4 h-4" /></button>
          </div>
        </div>
          ...
          <div className="w-full lg:w-[40%] flex flex-col justify-center order-2 lg:order-1 lg:pr-12 pt-8 lg:pt-0 z-20">
            <div className={cn("transition-all duration-500", isTransitioning ? "opacity-0 translate-y-4" : "opacity-100 translate-y-0")}>
              ...
              <h1 className="text-4xl sm:text-5xl lg:text-[65px] xl:text-[80px] font-black tracking-tighter uppercase leading-[0.85] text-[#111111] mb-8">
                {activeBag.title.split(' ').map((word, i, arr) => (
              ...
              <ul className="flex flex-col gap-3 mb-12">
                {activeBag.highlights.map((highlight, idx) => (
              ...
              <Link 
                href={generateWhatsAppLink(activeBag.whatsappMsg)}
                target="_blank"
                aria-label={`Enquire about ${activeBag.title} on WhatsApp`}
              ...
              <div className={cn("absolute inset-0 w-full h-full transition-all duration-700 flex items-center justify-center", isTransitioning ? "opacity-0 scale-95" : "opacity-100 scale-100")}>
                ...
                <div className="relative w-full max-w-[280px] md:max-w-[400px] xl:max-w-[480px] h-[350px] md:h-[500px] xl:h-[550px] flex items-center justify-center">
                  <Image
                    ...
                    fill
                    priority
                    className={cn("object-contain drop-shadow-2xl transition-transform duration-700 ease-out", activeBag.imageStyle)}
